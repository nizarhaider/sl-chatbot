...
setup() {
  ...
  local remote="root@$host" public_url runtime_env
  local -a connection=(-o BatchMode=yes -o StrictHostKeyChecking=accept-new -i "$SSH_KEY" -p "$port")
  test -f .env || fail "$ROOT/.env is missing"
...
apt-get update -qq
apt-get install -y -qq -o Dpkg::Options::="--force-confold" curl supervisor
...
[program:sl-webhook]
command=/workspace/sl-chatbot/.venv/bin/dotenv -f .env run -- .venv/bin/uvicorn app.main:app --host 0.0.0.0 --port $APP_PORT
...
rent() {
  load_vast
  local query offer result instance_id state host port
